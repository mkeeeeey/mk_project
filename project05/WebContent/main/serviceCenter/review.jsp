...
<script type="text/javascript">
	...
	function go(rev_no){
		$("[name=rev_no]").val(rev_no);
		$("[name=proc]").val("revDetail");
		$("form").submit();
	}
...
<div class="all-wrap">
	...
	<select name="proc">
		...
		<option value="review" selected>상품후기</option>
	...
	<c:forEach var="review" items="${rlist}" varStatus="status">
		<tr onclick="javascript:go(${review.rev_no})">
			<td align="center">${fn:length(rlist) - status.index}</td>
			<td>${review.rev_name}</td>
			<td align="center">${review.mem_id}</td>
			<td align="center">${review.rev_date}</td>
		</tr>
	</c:forEach>
	...
	<!-- 검색 + 글등록 -->
		...
		<tr style="border:none;">
			...
			<input id="writeRev" class="btn" type="button" value="글쓰기"/>
			<%-- </c:if> --%></th>
		</tr>
	</table>
	</form>
