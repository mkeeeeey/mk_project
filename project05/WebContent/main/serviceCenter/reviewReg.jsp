...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if(rev_detailp != null){
			...
			$("textarea").text(rev_detailp);
		}
		
		$("[name=uptRev]").click(function(){
			if(confirm("작성하시겠습니까?")){
				$("[name=proc]").val("uptRev");
				var rev_detail = $("[name=rev_detail]").val();
				rev_detail = rev_detail.replace(/(?:\r\n|\r|\n)/g, '<br>');
				$("[name=rev_detail]").val(rev_detail);
				$("form").submit();
			}
		});
		
		$("[name=insRev]").click(function(){
			if(confirm("작성하시겠습니까?")){
				$("[name=proc]").val("insRev");
				var rev_detail = $("[name=rev_detail]").val();
				rev_detail = rev_detail.replace(/(?:\r\n|\r|\n)/g, '<br>');
				$("[name=rev_detail]").val(rev_detail);
				$("form").submit();
...
<body>
	...
	<div class="cs-content">
		...
		<c:if test="${not empty param.rev_no}">
		...
		<div class="apply-cont">
			<table class="apply-table">
				...
				<td>
				<c:choose>
					<c:when test="${not empty param.rev_name}">
					<input type="text" name="rev_name" placeholder="제목을 입력해주세요" value="${param.rev_name}"/>
					</c:when>
					<c:otherwise>
					<input type="text" name="rev_name" placeholder="제목을 입력해주세요"/>
					</c:otherwise>
				</c:choose>
				</td></tr>
				<tr>
					<th>내용</th>
					<td>
						<textarea name="rev_detail" placeholder="내용을 입력해주세요"></textarea>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="apply-btn">
			<c:choose>
				<c:when test="${not empty param.rev_name}">
				<input class="btn" type="button" name="uptRev" value="수정"/>
				</c:when>
				<c:otherwise>
				<input class="btn" type="button" name="insRev" value="등록"/>
				</c:otherwise>
			</c:choose>
